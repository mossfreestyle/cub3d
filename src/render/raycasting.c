...
static void	perform_dda(t_info *info)
{
	...
	while (info->ray->hit == 0)
	{
		...
		if (info->map_info->final_map[info->ray->map_y][info->ray->map_x] == '1')
			info->ray->hit = 1;
	}
}
...
{
	if (info->ray->side == 0)
		info->ray->wall_dist = (info->ray->map_x - info->player->x 
			+ (1 - info->ray->step_x) / 2) / info->ray->dir_x;
	else
		info->ray->wall_dist = (info->ray->map_y - info->player->y 
